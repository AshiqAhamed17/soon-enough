...
export function PlaceStory({ place }: { place: Place }) {
  return (
    <motion.section
    ...
    >
      ...
      <p className="mt-4 text-lg leading-relaxed text-text">{place.whyItMatters}</p>

      <p className="mt-12 font-mono text-xs uppercase tracking-[0.3em] text-text-secondary/70">
        The story
      </p>
      <p className="mt-4 leading-relaxed text-text-secondary">{place.story}</p>
      ...
      <div className="relative mt-14 border-l-2 border-accent/30 pl-6">
        ...
        <p className="text-xl italic leading-snug text-text">{place.lesson}</p>
      </div>
    </motion.section>
  );
}
